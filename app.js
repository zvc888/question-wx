...
App({
  globalData: {
    baseAPI: "http://192.168.1.108:9430",
    // baseAPI: "https://web.zhentigo.com",
    limit: 10 //pageSize
  },
onLaunch: function() {
    let _this = this
    ...
    if (null == token || token == '') {
      wx.login({
        success(wxres) {
          if (wxres.code) {
            _this.formPost('/api/wx/user/login',{
              "code": wxres.code
            }).then(res=>{
              console.log(res)
              if(res && 20000 == res.code){
  ...
  formPost: function(url, data) {
    ...
    return new Promise(function(resolve, reject) {
      ...
      wx.request({
        ...
        data,
        success(res) {
          if ((res.statusCode !== 200 && res.statusCode !== 401) || typeof res.data !== 'object') {
            reject('网络出错')
            return false;
          }
